...
if [ -d "$DOTROOT/.local/bin" ] && ask_for_confirmation "Install scripts to ~/.local/bin?"; then
    find "$DOTROOT/.local/bin" -maxdepth 1 -type f -print0 | while IFS= read -r -d '' f; do
    ...
    echo "No dotfiles/.local/bin found or opted to skip."
fi

echo "✅ Done."
echo "Backups (this run): $backup_root"
